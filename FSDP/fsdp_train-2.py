...
class Config:
    ...
    # Smoke-test controls. Fixed-snippet validation still runs independently.
    build_tokenized_val_dataset: bool = False
...
def build_datasets(cfg: Config, tokenizer: Any) -> Tuple[Any, Optional[Any]]:
    ...
    # Skip tokenized validation split construction by default for smoke tests.
    # Periodic evaluate_fixed_snippet() validation still runs from validation_text.
    val_ds = None
    if cfg.build_tokenized_val_dataset:
...
def main() -> None:
    ...
    try:
        ...
        if is_main_process():
            logger.info(
                "Smoke-test controls | build_tokenized_val_dataset=%s | max_train_samples=%s | max_steps=%d",
                cfg.build_tokenized_val_dataset,
        ...
        if is_main_process():
            ...
            logger.info(
                "Final training state | final_checkpoint=%s | global_step=%d | save_optimizer=%s | save_rng_state=%s",
                final_ckpt,
                global_step,
                cfg.save_optimizer,
                cfg.save_rng_state,
